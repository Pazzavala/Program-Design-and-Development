...
void SimulationModel::CreateEntity(JsonObject& entity) {
  ...
  controller.AddEntity(*myNewEntity);entities.push_back(myNewEntity);
}

/// Schedules a trip for an object in the scene
void SimulationModel::ScheduleTrip(JsonObject& details) {std::string name = details["name"];
  ...
  for (auto entity : entities) {  // Add the entity to the scheduler
    ...
    if (name.compare(nameTemp) ==0 && typeTemp.compare("package") == 0 && entity->GetAvailability()) {
...
void SimulationModel::Update(double dt) {
  for (int i = 0; i < entities.size(); i++) {entities[i]->Update(dt, scheduler);
