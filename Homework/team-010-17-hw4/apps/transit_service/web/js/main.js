...
function notification(data) {
  notifbar = document.getElementById("notification-bar");
    ...
    // Msg to display: "Package #3/Customer #3: Maria_package has been scheduled for delivery to Maria.\n"
  if (data["value"] == "scheduled") {
    // notifbar.textContent += "Package #" + data["itemId"] + " & Customer #" + data[itemId] + ": " + data["package"] + " ":
    notifbar.textContent += "Package #" + data["itemId"] + 
    " has been scheduled for delivery to Customer #" + data["itemId"] + "." + "\r\n\r\n";
  }

  // Msg to display: "Drone #1: Moving to Package #1 using astar.\n"
  if (data["value"] == "moving") {
    notifbar.textContent += "Drone #" + data["droneId"] + ": Moving to deliver Package #" 
    + data["itemId"] + " to Customer #" + data["itemId"] + "." + "\r\n\r\n"; // + " using " + data["strategy"] + "\r\n";
  }

  // Msg to display: "Drone #1: Maria_package has been picked up.\n"
  if (data["value"] == "pickedup") {
    notifbar.textContent += "Drone #" + data["droneId"] + ": Package #" + data["itemId"] + " has been picked up." + "\r\n\r\n";
  }

  // Msg to display: "Drone #1: Maria_package has been delivered to Maria.\n"
  if (data["value"] == "delivered") {
    notifbar.textContent += "Drone #" + data["droneId"] + ": Package #" + data["itemId"]
     + " has been delivered to Customer #" + data["itemId"] + "." + "\r\n\r\n";
  }
}
