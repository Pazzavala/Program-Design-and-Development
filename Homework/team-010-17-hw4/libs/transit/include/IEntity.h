...
class IEntity {
  ...
  */
  ...
  /**
   * @brief Subscribes the observer to the entity
   * @param observer The observer that is subscribing
  */
  virtual void subscribe(std::string observerType, IObserver* observer);
  ...
  */
   ...
   * @param eventType The status update: scheduled, moving, picked up, delivered
   * @param this The entity that is sending the update
  */
  virtual void notify(std::string observerType, const IEntity& self);
  // changed name to self --> this keyword

  /**
   * @brief Finds what number drone the entity is
   * @return The drone's number
  */
  virtual int getSpecificId() const;
  ...
  */
  virtual int getPackageNumber() const;
 ...
 protected:
  ...
  std::string name;
  double speed = 0;
  std::unordered_map<std::string, std::vector<IObserver*>> observers;
};
