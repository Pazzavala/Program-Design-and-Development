...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  // Create a Observers for drone.
    ...
    PickedUpObserver* pickedUpObs = new PickedUpObserver(controller);
    DeliveredObserver* deliveredObs = new DeliveredObserver(controller);

    // Subscribe Observers to IEntity
    myNewEntity->subscribe("scheduled", scheduledObs);
    myNewEntity->subscribe("moving", movingObs);
    myNewEntity->subscribe("pickedup", pickedUpObs);
    myNewEntity->subscribe("delivered", deliveredObs);
...
// Maybe here we would remove the Observers???
